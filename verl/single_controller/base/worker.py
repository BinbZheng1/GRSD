...
class WorkerHelper:
    ...
    def _get_free_port(self):
        # NOTE: the OS ephemeral range on this host (10000-61000) leaves 18001-21269
        # unreserved, and PDC's orterun/logUpload watchers respawn periodically and
        # grab ports in the 18000-18100 band. A port picked here can be stolen by a
        # respawning orterun before torch.distributed binds it, causing EADDRINUSE.
        # Reject the contested band and retry to avoid that race.
        bad_lo, bad_hi = 18000, 18100
        for _ in range(64):
            with socket.socket() as sock:
                sock.bind(("", 0))
                port = sock.getsockname()[1]
            if not (bad_lo <= port <= bad_hi):
                return port
        return port
