...
case "${GRSD_VARIANT}" in
  external|original)
    export CHECKPOINT_DIR=${CHECKPOINT_DIR:-${REPO_DIR}/checkpoints/verl_agent_webshop/external_reflection_qwen2.5_7b}
    export EXPERIMENT_NAME=${EXPERIMENT_NAME:-GRSD-External-Reflection-Ablation-Qwen2.5-7B-WebShop}
    ;;
  *)
    export CHECKPOINT_DIR=${CHECKPOINT_DIR:-${REPO_DIR}/checkpoints/verl_agent_webshop/grsd_qwen2.5_7b}
    export EXPERIMENT_NAME=${EXPERIMENT_NAME:-GRSD-Qwen2.5-7B-WebShop}
    ;;
esac
